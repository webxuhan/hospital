...
//获得随机颜色值，参数a是透明值0~1
	...
	function show(){
		...
		for(var i=0;i<4;i++){
			...
			/*旋转的效果
				...
				context.rotate(Math.random()*Math.PI/3);
	...
	document.getElementsByTagName("button")[0].addEventListener("click",function(){
		...
		var userck = document.getElementsByTagName("input")[2];
		...
		if(!username.checkValidity()){
			username.setCustomValidity("请输入正确的用户名：^\w{3,16}$");
			return;
		}
